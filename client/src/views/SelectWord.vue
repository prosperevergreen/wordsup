		...
		<div class="mt-10">
			<v-card>
				<v-card-title class="title font-weight-regular justify-space-between">
					...
					<v-avatar color="purple darken-1" class="subheading white--text" size="24" v-text="step"></v-avatar>
				</v-card-title>
				<v-divider></v-divider>
					...
					<template v-for="(word, index) in wordDetails.slice(0, -1)">
						<v-window-item :value="word[word.length-1]" v-if="word.length > 1" :key="index">
							<v-card-text class="pa-1">
								<v-list>
									<v-list-item-group mandatory v-model="selected[index]" color="purple">
				...
				<v-card-actions>
					...
					<v-spacer></v-spacer>
					<v-btn :disabled="disableNext" class="white--text" color="purple" depressed @click="step++">
						Next
					</v-btn>
				</v-card-actions>
			</v-card>
